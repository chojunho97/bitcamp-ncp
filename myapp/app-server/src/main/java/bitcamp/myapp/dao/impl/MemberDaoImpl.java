...
public class MemberDaoImpl implements MemberDao {
  ...
  public void insert(Member m) {
    try (Statement stmt = con.createStatement()) {
      ...
      stmt.executeUpdate(
          sql,
          Statement.RETURN_GENERATED_KEYS //  insert 실행 수 자동 증가된 PK 값을 리턴 받겠다고 선언한다.
          );
      ...
      try (ResultSet keyRs = stmt.getGeneratedKeys()) {
        ...
        // 3) 가져오 PK 레코드에서 PK 값을 꺼낸다.
        int autoGeneratedmemberId = keyRs.getInt(1); // PK 컬럽이 한 개이기 때문에 PK 레코드에도 한 개의 컬럼만 있다.
